...
try:
    train_df, val_df, test_df, test_2_df = load_nlvr(return_test_2=True)
    print(
        f"Loaded {len(train_df)} train, {len(val_df)} val, {len(test_df)} test samples, and {len(test_2_df)} test_2 samples."
    )
except Exception as e:
...
test_dataset = NLVR2Dataset(test_df, processor, image_transform)
test_2_dataset = NLVR2Dataset(test_2_df, processor, image_transform)
...
checkpoints = [
    "checkpoints/vilt-nlvr2-no-adversarial/checkpoint-2700",
    "checkpoints/vilt-nlvr2-adversarial-image-0.01/checkpoint-2700",
    "checkpoints/vilt-nlvr2-adversarial-both-0.01/checkpoint-2700",
    "checkpoints/vilt-nlvr2-adversarial-text-0.03/checkpoint-2700",
]
...
for ckpt_path in checkpoints:
    ...
    trainer = AdversarialTrainer(
        adversarial_epsilon_text=0.0,
        adversarial_epsilon_image=0.0,
    ...
    test_2_result = trainer.evaluate(test_2_dataset)
    print(f"Test 2 results: {test_2_result}")
